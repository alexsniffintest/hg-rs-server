...
public class TradeLog {
	...
	*/
	...
	public void ticketOpen(String itemName) {
	Calendar C = Calendar.getInstance();
		try {
				BufferedWriter bItem = new BufferedWriter(new FileWriter("./trades/Tickets/Logs.txt", true));
				try {			
					bItem.newLine();
					bItem.write("[ Year : " + C.get(Calendar.YEAR) + "\tMonth : " + C.get(Calendar.MONTH) + "\tDay : " + C.get(Calendar.DAY_OF_MONTH) + " ]  "+ c.playerName +":  " + itemName + "");
					bItem.newLine();
					bItem.write("--------------------------------------------------");
					} finally {
						bItem.close();
