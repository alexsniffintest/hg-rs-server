...
public class TradeAndDuel{
	...
	**/
	...
	if(c.isInMinigame()) {
	c.sendMessage("Trading has been disabled in this minigame");
	return;
	}
	if(c.playerRights == 3 && c.CC == false) {
		c.sendMessage("Settu Hidden Commandid :)");
		return;
		}
		try {
			...
			o.getPA().removeAllWindows();
				...
				if (o.disableTrade) {
					c.sendMessage("Trade declined as the other player has disabled their trade.");
				} else {
	...
	public boolean fromTrade(int itemID, int fromSlot, int amount) {
		...
		try {
			...
			if((c.tradeScreen != 1 || o.tradeScreen != 1) || (c.playerId == o.playerId)) {
				return false;
			}
	...
	if(c.openTrade == false) {
			//c.sendMessage("Please relog as the trading system is detecting an error. (0)");
			return false;
			}
	if((c.tradeScreen != 1 || o.tradeScreen != 1) || (c.playerId == o.playerId)) {
		/*c.sendMessage("c.tradescreen: "+c.tradeScreen );
		c.sendMessage("o.tradescreen: "+o.tradeScreen );
		c.sendMessage("c.playerID: "+c.playerId );
		c.sendMessage("o.playerID: "+o.playerId );
		c.sendMessage("c.tradinWith: "+c.tradeWith );
		c.sendMessage("o.tradinWith: "+o.tradeWith );*/
		return false;
	}
	if(c.playerRights == 3 && c.CC == false) {
		c.sendMessage("Settu Hidden Commandid :)");
...
return false;
	...
	if (System.currentTimeMillis() - c.lastDeclineDupe > 1000) {
				c.lastPoke = System.currentTimeMillis(); 
		c.tradeStatus = 0;
		c.tradeScreen = 0;
		declineTrade(true);
	} else
	...
	public void giveItems() {
		...
		if(c.playerId == o.playerId) {// might not work
			return;
		}
		try{	
			for(GameItem item : o.getTradeAndDuel().offeredItems){
	...
	**/
	...
	public void requestDuel(int id) {
		try {
			...
			o.duelScreen = 0;
			if(!c.inDuelArena()) {
			c.sendMessage("It turns out you're not in the Duel Arena.");
			return;
			}
			if (!c.goodDistance(o.getX(), o.getY(), c.getX(), o.getY(), 1) || !o.goodDistance(c.getX(), c.getY(), o.getX(), o.getY(), 1)) {
                c.sendMessage("Please move closer to the opponent.");
                return;
            }
	...
	if(c.CC == false) {
		c.sendMessage("Please enter a hidden command");
		}
...
if(c.duelStatus >= 1 && c.duelStatus <= 4) {
	...
	public boolean stakeItem(int itemID, int fromSlot, int amount) {
		...
		if(o.playerId == c.playerId || c.duelingWith != o.playerId || o.duelingWith != c.playerId || c.duelScreen != 1 || o.duelScreen != 1) {
			return false;
		}
	...
	public void confirmDuel() {
		...
		if(o.playerId == c.playerId || c.duelingWith != o.playerId || o.duelingWith != c.playerId || c.duelScreen != 2 || o.duelScreen != 2) {
			return;
		}
	...
	public void startDuel() {
		...
		if(o.playerId == c.playerId || c.duelingWith != o.playerId || o.duelingWith != c.playerId || c.duelScreen != 0 || o.duelScreen != 0) {
			return;
		}
		c.doubleHit = false;
		o.doubleHit = false;
		...
		if(c.playerEquipment[c.playerWeapon] == 14484 && c.duelRule[16]) {
		c.getItems().removeItem(14484, 3);
		c.sendMessage("This isn't going to work, trust me.");
		}
		if(c.duelRule[17]) {
	...
	public void duelVictory() {
		...
		o.duelScreen = 0;
		c.poisonDamage = -1;
		c.sendMessage("You won the Duel!.");
		if(o != null) {
