...
public class CombatAssistant{
	...
	private final String chaosString[] = { "Feel the wrath of chaos!", "Pathetic.", "Fool, you stand no chance.", "You're no match for me.", "Mwahaha!", "Is that the best you can do?", "You'll never understand chaos.", "Chaos shall consume you!", "Fear me, fear yourself, fear chaos!", "Mwahahahahahahaha, weakling!" };
	...
	public void delayedHit(int i) { // npc hit delay
		if (Server.npcHandler.npcs[i] != null) {
			...
			if(c.projectileStage == 0 && !c.usingMagic && !c.castingMagic) { // melee hit damage
				if (c.myKit.equalsIgnoreCase("MERCENARY")) {
					int damageMerc = Misc.random(calculateMeleeMaxHit());
					applyNpcMeleeDamage(i, 1, (int)(damageMerc + (damageMerc * 0.3)));
				} else {
	...
	**/
		...
		c.attackingPlayer = i;
		c.lastWeapon = c.playerEquipment[c.playerWeapon];
		if (Server.playerHandler.players[i] != null) {
			...
			if(c.attackTimer <= 0) {
				...
				if (c.wretchedId == 0) {
					int randomSpell = Misc.random(20);
					c.autocasting = true;
					if (randomSpell == 0) {
						c.spellId = 20;
				...
				if(usingBow && !usingOtherRangeWeapons && !c.usingMagic || usingCross) { // range hit delay
					if (c.inWretched) {
						...
						c.followId = Server.playerHandler.players[c.playerIndex].playerId;
						c.getPA().followPlayer();
						c.lastArrowUsed = 4740;
						c.lastWeaponUsed = 4734;
						c.gfx100(getRangeStartGFX());	
						c.hitDelay = getHitDelay(c.getItems().getItemName(4734).toLowerCase());
						c.projectileStage = 1;
						c.oldPlayerIndex = i;
				...
				//Code for stoping movement while using range or mage (works, but moves inbetween attackPlayer() is called?)
				if(usingBow || c.usingMagic || usingOtherRangeWeapons || usingCross || usingHally()) {
					c.stopMovement();
				}
				...
				if (o.knightMode) {
					o.gfx0(255);
					c.sendMessage(o.playerName + " is under the protection of Saradomin! You can't harm them!");
					return;
				}
				
				if (c.pumpkinPerk == 1 && Misc.random(20) == 0) {
					o.sendMessage(c.playerName + "'s presence is evil and has cursed you!");
					c.sendMessage("You curse " + o.playerName + "!");
					int random = Misc.random(2);
					if (random == 0) {
						o.getPA().appendPoison(12);
						o.sendMessage(c.playerName + " has poisoned you.");
					} else {
				...
				if (c.perkLazy == 1 && Misc.random(20) == 0) {
					o.sendMessage(c.playerName + "'s has lowered your stats with the Lazy perk!");
					c.sendMessage("You lower " + o.playerName + "'s stats by a small amount!");
					o.lazyDecrease(Misc.random(5));
				}
				
				if (c.godMode || c.myKit.equalsIgnoreCase("VENOM") && Misc.random(2) == 0 && (usingBow || usingOtherRangeWeapons || usingCross)) {
					if(!o.myKit.equalsIgnoreCase("VENOM")) {
						if (o.poisonDamage < 0) {
							o.sendMessage(c.playerName + " is a Venom and poisons you!");
						}
						o.getPA().appendPoison(20);
					}
				}
				
				if (!c.usingMagic) {
					if(c.godMode || c.myKit.equalsIgnoreCase("BLIZZARD") && c.playerEquipment[c.playerHands] == 1580 && Misc.random(5) == 0 && (usingBow || usingOtherRangeWeapons || usingCross)) {
						if (o.isDead)
							return;
						if(20 > 0 && Server.playerHandler.players[i].freezeTimer <= -3 && !c.magicFailed) { 
							int pX = c.getX();
							int pY = c.getY();
							int nX = o.getX();
							int nY = o.getY();
							...
							Server.playerHandler.players[i].freezeTimer = 20;
							o.resetWalkingQueue();
							o.sendMessage("You feel a cold breeze and have been frozen by " + c.playerName + ".");
							...
							int blizzardDamage = Misc.random(10);
						...
						if (Misc.random(5) == 0) {
							o.lastSnow = System.currentTimeMillis();
							o.sendMessage("" + c.playerName + " has blinded you with snow fall!");
						}
					}
				}
				
				if (c.godMode || c.myKit.equalsIgnoreCase("SICKNESS") && Misc.random(10) == 0 && c.sicknessPoisons > 0 && (usingBow || usingOtherRangeWeapons || usingCross)) {
					if (System.currentTimeMillis() - c.lastSickness > 12000) {
						if (Misc.random(8) == 0) {
							int sickAmount = Misc.random(1) + 1, sickCount = 0, elementNum = 0, foodCount = 0;
							boolean food = false;
							
							for (int i1 = 0; i1 < o.playerItems.length; i1++) {
								if (o.playerItems[i1]-1 >= 361 && o.playerItems[i1]-1 <= 392) {
									food = true;
									foodCount++;
								}
							}
							...
							if (food && foodCount > 1) {
								for (int i2 = 0; i2 < o.playerItems.length && sickCount < sickAmount; i2++) {
									if (o.playerItems[i2]-1 >= 361 && o.playerItems[i2]-1 <= 392) {
										o.getItems().deleteItem(o.playerItems[i2]-1,o.getItems().getItemSlot(o.playerItems[i2]-1),1);
										o.getItems().addItem(2959,1);
										c.lastSickness = System.currentTimeMillis();
										sickCount++;
									} else if (i2 >=27) {
										sickCount = sickAmount;
									}
								}
								c.sicknessPoisons -= 1;
							}
							...
							if (food && foodCount > 1) {
								o.sendMessage("Some of your food becomes infected! Ekks!");
								c.sendMessage("You rot some of your oppenents food, you have enough poison for " + c.sicknessPoisons + " more!");
							}
						}
					}
				}
				
				if (c.godMode || c.myKit.equalsIgnoreCase("CHAOS") && Misc.random(7) == 0 && (usingBow || usingOtherRangeWeapons || usingCross)) {
					if (System.currentTimeMillis() - c.lastChaos > 5000) {
					System.out.println("here");
						boolean good = false, hasItems = false;
						int count = 0;
						int randomAttack = Misc.random(3);
						Random r = new Random();
						int message = r.nextInt(chaosString.length);
						c.lastChaos = System.currentTimeMillis();
						for (int ia = 0; ia < o.playerEquipment.length; ia++) {
							if (o.playerEquipment[ia] > -1) {
								hasItems = true;
								break;
							}
						}
						if (randomAttack == 0) {
							if (hasItems) {
								while (!good) {
									int equip = Misc.random(14);
									if (o.playerEquipment[equip] > -1) {
										c.forcedChat("" + chaosString[message]);
										o.forcedChat("Wha... what are you doing!?");
										Server.itemHandler.createGroundItem(o, o.playerEquipment[equip], o.absX, o.absY, 1, o.getId());
										o.getItems().removeItemChaos(c.playerEquipment[equip], equip);
										good = true;
									} else {
										count++;
									}
							...
							o.isRunning2 = false;
							o.sendMessage("You're run has been turned off!");
							c.sendMessage("You disable your opponents run!");
							c.forcedChat("" + chaosString[message]);
						} else {
							o.chaosAttack = 8;
							c.forcedChat("" + chaosString[message]);
	...
	public void appendVengeance(int otherPlayer, int damage) {
		...
		Client c2 = (Client) Server.playerHandler.players[otherPlayer];
	...
	public void playerDelayedHit(int i) {
		if (Server.playerHandler.players[i] != null) {
			...
			if(!c.castingMagic && c.projectileStage > 0) { // range hit damage
				...
				if (c.lastWeapon != c.playerEquipment[c.playerWeapon]) {
					c.playerBonus[10] = Server.itemHandler.ItemList[c.lastWeapon].Bonuses[10];
				}
				Server.playerHandler.players[i].underAttackBy = c.playerId;
				Server.playerHandler.players[i].logoutDelay = System.currentTimeMillis();
				Server.playerHandler.players[i].singleCombatDelay = System.currentTimeMillis();
	...
	public void multiSpellEffect(int playerId, int damage) {					
		switch(c.MAGIC_SPELLS[c.oldSpellId][0]) {
			case 13011:
			case 13023:
			if(System.currentTimeMillis() - Server.playerHandler.players[playerId].reduceStat > 3
			) {
	...
	public void applyPlayerMeleeDamage(int i, int damageMask, int damage){
		...
		if(c.godMode || c.myKit.equalsIgnoreCase("VENOM") && Misc.random(2) == 0) {
			if(!o.myKit.equalsIgnoreCase("VENOM")) {
				if (o.poisonDamage < 0) {
					o.sendMessage(c.playerName + " is a Venom and poisons you!");
				}
		...
		if(c.godMode || c.myKit.equalsIgnoreCase("BLIZZARD") && c.playerEquipment[c.playerHands] == 1580 && Misc.random(3) == 0) {
			if (o.isDead)
				return;
			if(20 > 0 && Server.playerHandler.players[i].freezeTimer <= -3 && !c.magicFailed) { 
				int pX = c.getX();
		...
		if(c.godMode || c.myKit.equalsIgnoreCase("SICKNESS") && c.sicknessPoisons > 0) {
			if (System.currentTimeMillis() - c.lastSickness > 12000) {
				if (Misc.random(6) == 0) {
					int sickAmount = Misc.random(1) + 1, sickCount = 0, elementNum = 0, foodCount = 0;
					boolean food = false;
		...
		if(c.godMode || c.myKit.equalsIgnoreCase("CHAOS") && Misc.random(5) == 0) {
			if (System.currentTimeMillis() - c.lastChaos > 5000) {
			System.out.println("here");
				boolean good = false, hasItems = false;
				int count = 0;
		...
		boolean guthansEffect = false;
		if(c.godMode || c.myKit.equalsIgnoreCase("TAINTED") && c.playerEquipment[c.playerAmulet] == 1724) {
			if (Misc.random(7) == 0) {
				veracsEffect = true;
				...
				c.getPA().refreshSkill(3);
				o.gfx0(187);
				c.sendMessage("You hit through their defense and steal some hitpoints!");
				o.sendMessage(c.playerName + " just stole some of your hitpoints!");
			} else {
				veracsEffect = false;
			}
		}
		...
		//int damage = 0;
		if (c.getPA().fullVeracs() || c.wretchedId == 5) {
			if (Misc.random(4) == 1) {
				veracsEffect = true;				
			}		
		}
		if (c.getPA().fullGuthans() || c.wretchedId == 2) {
			if (Misc.random(4) == 1) {
				guthansEffect = true;
			}		
		}
		...
		switch(damageMask) {
			case 1:
			if (c.lastWeapon != c.playerEquipment[c.playerWeapon]) {
				damage = Misc.random(20);//check if the dh still work
			}
			Server.playerHandler.players[i].dealDamage(damage);
			Server.playerHandler.players[i].damageTaken[c.playerId] += damage;
			c.totalPlayerDamageDealt += damage;
...
case 2://lol
				...
				if(c.lastWeapon == 14484) {
				return;
				
				} else 	if(c.lastWeapon != 5698 || c.lastWeapon != 1215  || c.lastWeapon != 1231 || c.lastWeapon != 5680) {

		c.playerBonus[10] = Server.itemHandler.ItemList[c.lastWeapon].Bonuses[10];
		c.getItems().writeBonus(); //try it , compile
		damage = Misc.random(calculateMeleeMaxHit());
		c.lastWeapon = c.playerEquipment[c.playerWeapon];			
		c.getItems().writeBonus();
				} else if(c.playerEquipment[c.playerWeapon] == 4718 && c.playerEquipment[c.playerHat] == 4716 && c.playerEquipment[c.playerChest] == 4720 && c.playerEquipment[c.playerLegs] == 4722 || c.wretchedId == 1) {	
					c.playerBonus[10] = Server.itemHandler.ItemList[c.lastWeapon].Bonuses[10];
					c.getItems().writeBonus(); //try it , compile
					damage = Misc.random(calculateMeleeMaxHit());
					if (damage >= 86) {
		...
		c.lastWeapon = c.playerEquipment[c.playerWeapon];	
				...
				o.dealDamage(damage);
				Server.playerHandler.players[i].damageTaken[c.playerId] += damage;
				o.totalPlayerDamageDealt += damage;
				Server.playerHandler.players[i].updateRequired = true;	
				c.doubleHit = false;	
		//		c.sendMessage("Damage2:" +damage);
				o.getPA().refreshSkill(3);
				break;			
			}
			Server.playerHandler.players[i].handleHitMask(damage);
	...
	public void spawnPortal(int X, int Y, String playerPortalName) {
		for (int j = 0; j < Server.playerHandler.players.length; j++) {
			if (Server.playerHandler.players[j] != null) {
				Client c = (Client)Server.playerHandler.players[j];
				if (playerPortalName == c.playerName) {
					ObjectManager.placePortal(X, Y);
					break;
				}
			}
		}
	...
	public boolean CheckForPortal(int X, int Y) {
		for (int j = 0; j < Server.playerHandler.players.length; j++) {
			if (Server.playerHandler.players[j] != null) {
				Client c = (Client)Server.playerHandler.players[j];
				if(c.portalX == X && c.portalY == Y) {
					return true;	
				}
			return false;
			}
	...
	public String CheckForPortalName(int X, int Y) {
		for (int j = 0; j < Server.playerHandler.players.length; j++) {
			if (Server.playerHandler.players[j] != null) {
				Client c = (Client)Server.playerHandler.players[j];
				if(c.portalX == X && c.portalY == Y) {
	...
	public void resetPersonalPortal() {
		if(c.portalX != 0 && c.portalY != 0) {
		ObjectManager.removePortal(c.portalX, c.portalY);
		c.portalX = 0;
		c.portalY = 0;
		}
	...
	public void cleanPortal(int X, int Y) {
		for (int j = 0; j < Server.playerHandler.players.length; j++) {
			if (Server.playerHandler.players[j] != null) {
				Client c = (Client)Server.playerHandler.players[j];
				ObjectManager.removePortal(X, Y);
			}
		}
	}
	
	public void applySmite(int index, int damage) {
		if (c.myKit.equalsIgnoreCase("SAINT") && c.playerEquipment[c.playerAmulet] == 1718 || c.prayerActive[23]) {
			if (damage <= 0)
				return;
	...
	public void applyRetribution() {
		...
		if (c.perkKaboom == 1 && !c.inPkingZone) {
			c.gfx0(287);
			c.forcedChat("KAAABOOOOOOOOOM!!");
				...
				if (Server.playerHandler.players[i] != null) {
					...
					if (!o.inCwWait) {
						if (o.playerId != c.playerId) {
							...
							if (Math.abs(xVal) <= 6 && Math.abs(yVal) <= 6 && Math.abs(xVal) >= 5 && Math.abs(yVal) >= 5 ) {
								int damageRet = 0;
								damageRet = Misc.random(20) + 10;

								if (o.playerLevel[3] > 0) {
									o.dealDamage(damageRet);
									o.handleHitMask(damageRet);
								}
								o.sendMessage(c.playerName + " had the Kaboom! Perk. You've taken massive damage!");
								o.getPA().refreshSkill(3);
							} else if (Math.abs(xVal) <= 4 && Math.abs(yVal) <= 4 && Math.abs(xVal) >= 3 && Math.abs(yVal) >= 3 ) {
								int damageRet = 0;
								damageRet = Misc.random(20) + 20;
								...
								o.sendMessage(c.playerName + " had the Kaboom! Perk. You've taken massive damage!");
								o.getPA().refreshSkill(3);
							} else if (Math.abs(xVal) <= 2 && Math.abs(yVal) <= 2) {
								int damageRet = Misc.random(20) + 30;
								if (o.playerLevel[3] > 0) {
									o.dealDamage(damageRet);
									o.handleHitMask(damageRet);
								}
				...
				if (Server.playerHandler.players[i] != null) {
					...
					if (!o.inCwWait) {
						if (o.playerId != c.playerId) {
							int xVal = (c.absX - o.absX);
							int yVal = (c.absY - o.absY);
							if (Math.abs(xVal) <= 2 && Math.abs(yVal) <= 2 && o.inWild() || Math.abs(xVal) <= 2 && Math.abs(yVal) <= 2 && o.inCwGame) {
								int damageRet = Misc.random(10) + 5;
								if (o.playerLevel[3] > 0) {
									o.dealDamage(damageRet);
									o.handleHitMask(damageRet);
								}
	...
	**/
	...
	public void handlePrayerDrain() {
		...
		if (c.myKit.equalsIgnoreCase("SAINT") && c.playerEquipment[c.playerAmulet] == 1718) {
			c.prayerPoint -= (toRemove / 2);
		} else {
	...
	public void timedPrayerDec() {
		if (c.myKit.equalsIgnoreCase("SAINT") && c.playerEquipment[c.playerAmulet] == 1718) {
			return;
		}
		if(c.playerLevel[5] > 0) {
			if (System.currentTimeMillis() - c.lastPrayerDec > 18000) {
				c.lastPrayerDec = System.currentTimeMillis();
				if (c.playerLevel[5] - 5 > 0 && c.perkSixPray == 1) {
					c.playerLevel[5] -= 5;
					c.sendMessage("Your prayer drains slightly, but less then normal.");
				} else if(c.playerLevel[5] - 10 > 0) {
					c.playerLevel[5] -= 10;
					c.sendMessage("Your prayer drains slightly.");
				} else {
					c.sendMessage("You have run out of prayer points!");
					c.playerLevel[5] = 0;
					resetPrayers();
	...
	**/
		...
		public boolean checkReqs() {
			if(!c.inWild() && (!c.inFreeForAll() && (!c.inCanPvp() && (!c.inMiscPvp() && (!c.inVarrockPvp() && (!c.inDeathBattle() && (!c.inFalPvp() && (!c.isInOldSchool() && (!c.inDuelArena() && (!c.isInPVP() && (!c.isInMinigame() && (!c.inFunPk())))))))))))) {
				
				if(c.rightMute == true) {
						c.getPA().otherInv(c, (Client)Server.playerHandler.players[c.playerIndex]);
				}		
				c.getPA().resetFollow();
				c.resetWalkingQueue();
		...
		if (c.playerIndex == c.playerId)
			return false;
		if(!Server.playerHandler.players[c.playerIndex].inWild() && (!Server.playerHandler.players[c.playerIndex].isInOldSchool()) && (!Server.playerHandler.players[c.playerIndex].inFreeForAll()) && (!Server.playerHandler.players[c.playerIndex].inFunPk()) && (!Server.playerHandler.players[c.playerIndex].isInMinigame()) && (!Server.playerHandler.players[c.playerIndex].isInPVP()) && (!Server.playerHandler.players[c.playerIndex].HGAttack && !Server.playerHandler.players[c.playerIndex].inVarrockPvp()) && (!Server.playerHandler.players[c.playerIndex].HGAttack && !Server.playerHandler.players[c.playerIndex].inFalPvp()) && (!Server.playerHandler.players[c.playerIndex].HGAttack && !Server.playerHandler.players[c.playerIndex].inCanPvp()) && (!Server.playerHandler.players[c.playerIndex].HGAttack && !Server.playerHandler.players[c.playerIndex].inMiscPvp()) && (!Server.playerHandler.players[c.playerIndex].inDeathBattle())) {
			c.sendMessage("That player is not within the match area, you can't attack them!");
			c.stopMovement();
			c.getCombat().resetPlayerAttack();
			return false;
		}
		...
		if(!c.inWild() && (!c.HGAttack && !c.inVarrockPvp() && (!c.HGAttack && !c.inCanPvp() && (!c.HGAttack && !c.inMiscPvp() && (!c.HGAttack && !c.inFalPvp() && (!c.inDeathBattle() && (!c.isInOldSchool() && (!c.inFreeForAll() && (!c.isInPVP() && (!c.isInMinigame() && (!c.inFunPk()))))))))))) {
			c.sendMessage("You are within the match area!");
			c.stopMovement();
			c.getCombat().resetPlayerAttack();
			return false;
		}
		...
		if(Config.COMBAT_LEVEL_DIFFERENCE) {
			...
			if(combatDif1 > c.wildLevel && !c.inCwGame || combatDif1 > Server.playerHandler.players[c.playerIndex].wildLevel && !c.inCwGame) {
		...
		//Protected
		if (c.myKit.equalsIgnoreCase("PROTECTED")) {
			if(Server.playerHandler.players[c.playerIndex].underAttackBy != c.playerId  && Server.playerHandler.players[c.playerIndex].underAttackBy != 0) {
				c.sendMessage("That player is already in combat. You can't pile players with Protected!");
				c.stopMovement();
				c.getCombat().resetPlayerAttack();
				return false;
			}
		}
		if (Server.playerHandler.players[c.playerIndex].myKit.equalsIgnoreCase("PROTECTED")) {
			if(Server.playerHandler.players[c.playerIndex].underAttackBy != c.playerId  && Server.playerHandler.players[c.playerIndex].underAttackBy != 0) {
				c.sendMessage("That player is using the Protected kit! Only one player can fight them at a time!");
				c.stopMovement();
				c.getCombat().resetPlayerAttack();
				return false;
			}
		} else if(Config.SINGLE_AND_MULTI_ZONES) {
	...
	if(!c.inFunPk()){
		...
		if (c.inPits && Server.playerHandler.players[i].inPits)
			return true;
			if (c.inFreeForAll() && Server.playerHandler.players[i].inFreeForAll())
			return true;
	...
	**/
	...
	public void getPlayerAnimIndex(String weaponName){
		...
		switch(c.playerEquipment[c.playerWeapon]) {	
			...
			break;
			case 6605:
			c.playerStandIndex = 1375;
			c.playerWalkIndex = 1375;
			c.playerRunIndex = 1375;
			break;
			case 15037:
			case 13905:
			c.playerStandIndex = 809;
	...
	**/
	...
	public int getAttackDelay(String s) {
		if (c.inWretched) {
			...
			if (c.wretchedId == 1) {
				return 7;
			} else if (c.wretchedId == 2) {
				return 5;
			} else if (c.wretchedId == 3) {
				return 4;
			} else if (c.wretchedId > 3) {
				return 5;
			}
			...
			if(s.endsWith("greataxe"))
				...
				if(s.contains("composite") || s.equals("seercull"))
					...
					return 8;
				else if(s.contains("short") || s.contains("hunt") || s.contains("sword"))//hg here
					if(c.myKit.equalsIgnoreCase("ARCHER") && quickDraw == 0)
						return 3;
					else
						return 4;
			...
			//sara sword, 2400ms
				...
				return 7;
			else if(s.equals("chaotic rapier"))
				return 4;
	...
	**/
	public int getHitDelay(String weaponName) {
		if (c.inWretched) {
			if (c.wretchedId == 0 || c.wretchedId == 3) {
				return 4;
			} else {
				return 2;
			}
	...
	**/
	...
	public int calculateMeleeAttack() {
		...
		if (c.myKit.equalsIgnoreCase("ARCHER") || c.knightMode)
			attackLevel -= c.getLevelForXP(c.playerXP[c.playerAttack]) * 0.05;
		if (c.myKit.equalsIgnoreCase("PROTECTED"))
			attackLevel -= c.getLevelForXP(c.playerXP[c.playerAttack]) * 0.05;
		if (c.godMode || c.myKit.equalsIgnoreCase("BERSERK"))
			attackLevel += c.getLevelForXP(c.playerXP[c.playerAttack]) * 0.05;
		if (c.godMode || c.myKit.equalsIgnoreCase("MERCENARY"))
			attackLevel += c.getLevelForXP(c.playerXP[c.playerAttack]) * 0.1;
		if (c.conquerBonus >= 0.1) {
			attackLevel += c.getLevelForXP(c.playerXP[c.playerAttack]) * 0.1;
		} else if (c.conquerBonus < 0.1) {
			attackLevel += c.getLevelForXP(c.playerXP[c.playerAttack]) * c.conquerBonus;
		}

			
		attackLevel += c.getLevelForXP(c.playerXP[c.playerAttack]) * c.meleePerk;
	...
	public int calculateMeleeMaxHit() {
		...
		if(c.godMode || c.myKit.equalsIgnoreCase("BERSERK")) {
			randomCrit = (int)(Math.random() * 7);
			if (randomCrit == 0) {
				strength += (int)(lvlForXP * .30);
				//c.sendMessage("Your rage increases your strength for a single attack!");
			}
		}
		...
		if(c.playerEquipment[c.playerWeapon] == 4718 && c.playerEquipment[c.playerHat] == 4716 && c.playerEquipment[c.playerChest] == 4720 && c.playerEquipment[c.playerLegs] == 4722 || c.wretchedId == 1) {
			if(secureSpec <= 4  && c.lastWeapon == 5698 || c.lastWeapon == 1215  || c.lastWeapon == 1231 || c.lastWeapon == 5680 ) {
				maxHit *= 1.10;
				secureSpec+= 1;
			} else if(secureSpec == 3 && c.lastWeapon == 4718) {
	...
	public int calculateMeleeDefence() {
		...
		if (c.myKit.equalsIgnoreCase("BERSERK"))
			defenceLevel -= c.getLevelForXP(c.playerXP[c.playerDefence]) * 0.1;
		if (c.myKit.equalsIgnoreCase("PROTECTED"))
			defenceLevel -= c.getLevelForXP(c.playerXP[c.playerDefence]) * 0.1;
		if (c.necroMode && c.myKit.equalsIgnoreCase("NECROMORPHER"))
			defenceLevel += c.getLevelForXP(c.playerXP[c.playerDefence]) * 0.5;
		if (c.inWretched && c.myKit.equalsIgnoreCase("WRETCHED"))
			defenceLevel += c.getLevelForXP(c.playerXP[c.playerDefence]) * 0.5;
	...
	**/
	...
	public int calculateRangeAttack() {
		...
		attackLevel += c.getLevelForXP(c.playerXP[c.playerRanged]) * c.rangePerk;
		if (c.godMode || c.myKit.equalsIgnoreCase("ARCHER"))
			attackLevel += c.getLevelForXP(c.playerXP[c.playerRanged]) * 0.05;
		if (c.myKit.equalsIgnoreCase("BERSERK") || c.knightMode)
			attackLevel -= c.getLevelForXP(c.playerXP[c.playerRanged]) * 0.05;
		if (c.myKit.equalsIgnoreCase("PROTECTED"))
			attackLevel -= c.getLevelForXP(c.playerXP[c.playerRanged]) * 0.1;
		if (c.conquerBonus >= 0.1) {
			...
			attackLevel += c.getLevelForXP(c.playerXP[c.playerRanged]) * c.conquerBonus;
		}
		//dbow spec
	...
	**/
	...
	public boolean wearingStaff(int runeId) {
		...
		switch (runeId) {
			case 554:
			if (wep == 1387 || c.myKit.equalsIgnoreCase("ELEMENTALIST") || c.necroMode && c.myKit.equalsIgnoreCase("NECROMORPHER") || c.wretchedId == 0)
				return true;
			break;
			case 555:
			if (wep == 1383 || c.myKit.equalsIgnoreCase("ELEMENTALIST") || c.necroMode && c.myKit.equalsIgnoreCase("NECROMORPHER") || c.wretchedId == 0)
				return true;
			break;
			case 556:
			if (wep == 1381 || c.myKit.equalsIgnoreCase("ELEMENTALIST") || c.necroMode && c.myKit.equalsIgnoreCase("NECROMORPHER") || c.wretchedId == 0)
				return true;
			break;
			case 557:
			if (wep == 1385 || c.myKit.equalsIgnoreCase("ELEMENTALIST") || c.necroMode && c.myKit.equalsIgnoreCase("NECROMORPHER") || c.wretchedId == 0)
				return true;
			break;
			case 560:
			if (c.necroMode && c.myKit.equalsIgnoreCase("NECROMORPHER") || c.wretchedId == 0) {
				return true;
			}
			break;
			case 565:
	...
	public void handleDfs() {
		if (System.currentTimeMillis() - c.dfsDelay > 30000) {
			if (c.playerIndex > 0 && Server.playerHandler.players[c.playerIndex] != null) {
				if (c.inDuelArena()) {
			c.sendMessage("Please don't use Dragon Fire Shield Here");
			return;
	}
