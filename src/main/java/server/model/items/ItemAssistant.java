...
public class ItemAssistant {//wepon
	...
	**/
	...
	public void getBonus() {
		if (c.inWretched && c.inCwGame) {
			double timedBonus = 0;
			if (c.randomMap == 0 && !c.startDeathBattle) {
				timedBonus = ((Server.HungerGames.gameTimer * .1) / 50) + 0.2;
			} else if (c.randomMap == 1 && !c.startDeathBattleFal) {
				timedBonus = ((Server.HungerGamesFal.gameTimer * .1) / 47) + 0.2;
			} else if (c.randomMap == 2 && !c.startDeathBattleCan) {
				timedBonus = ((Server.HungerGamesCan.gameTimer * .1) / 44) + 0.2;
			} else if (c.randomMap == 3 && !c.startDeathBattleMisc) {
				timedBonus = ((Server.HungerGamesMisc.gameTimer * .1) / 47) + 0.2;
			} else {
				timedBonus = 1.10;
			}
			int display = (int) (timedBonus * 100);
			c.sendMessage("Your stats bonus is " + display + "% of its normal state.");
			for (int i = 0; i < c.wretchedBonus.length; i++) {
				if (c.wretchedBonus[i] > -1) {
					for (int j = 0; j < Config.ITEM_LIMIT; j++) {
						if (Server.itemHandler.ItemList[j] != null) {
								if (Server.itemHandler.ItemList[j].itemId == c.wretchedBonus[i]) {
								for (int k = 0; k < c.playerBonus.length; k++) {
									int newBonus = (int) (Server.itemHandler.ItemList[j].Bonuses[k] * timedBonus);
									c.playerBonus[k] += newBonus;
									c.lastCheck = System.currentTimeMillis();
								}
								break;
							}
						}
	...
	**/
	...
	public void sendWeapon(int Weapon, String WeaponName) {
		...
		if (WeaponName.equals("Unarmed")) {
		...
		} else if (WeaponName2.startsWith("dagger") || WeaponName2.contains("anchor") || WeaponName2.contains("sword") || WeaponName2.contains("Excalibur")) {
			c.setSidebarInterface(0, 2276); //stab, lunge, slash, block
			c.getPA().sendFrame246(2277, 200, Weapon);
			c.getPA().sendFrame126(WeaponName, 2279);
			...
			c.getPA().sendFrame246(8461, 200, Weapon);
			c.getPA().sendFrame126(WeaponName, 8463);
		} else if (WeaponName2.startsWith("spear") || WeaponName2.startsWith("Vesta's Spear") || WeaponName2.startsWith("Dildo")) {
...
//rc pouchs
			...
			**/
			case 13867:	
			c.magicLevelReq = 78;
			c.attackLevelReq = 78;//zuriel staff
			break;	
			case 13858:	
			case 13864:	
			case 13861:	
	...
	**/
	...
	public boolean wearItem(int wearID, int slot) {
		synchronized(c) {
			...
			if(c.openTrade || c.openDuel) {
				return false;
			}
			
			if (wearID == 88 && System.currentTimeMillis() - c.lastFlash < 8000) {//lightness
				long getTime = System.currentTimeMillis() - c.lastFlash;
				long finalTime = 8 - getTime / 1000;
				c.sendMessage("You have " + finalTime + " seconds till you can use your flash again!");
				return false;
			}
			
			if (wearID == 9813 || wearID == 9814) {
				int count = 0;
				for (int i = 0; i < c.achievements.length; i++) {
					if (c.achievements[i][0] == 1) {
						count++;
					}
				}
				if (count != c.achievements.length) {
					c.sendMessage("You must complete all of the Achievements before wearing this!");
					return false;
				}
			}
			...
			if(wearID == 7927) {
				if (c.inPkingZone()) {
					c.sendMessage("You can't use this item here!");
					return false;
				}
			...
			if(wearID == 6583) {
				...
				if(c.getItems().getItemName(wearID).contains("scarf")) {
					targetSlot=2;
				}	
				...
				if (wearID == 2661) {
					c.sendMessage("This item is bugged atm, turn this into one of the mods for a reward,");
					c.sendMessage("or keep it till it gets fixed.");
					return false;
				}
				...
				if (wearID == 4002) {
					if (c.myKit.equalsIgnoreCase("TAGGED") && c.inCwGame) {
						...
						if (c.chestRoomTimer > 0) {
							c.sendMessage("You can't do that from in here!");
							return false;
						}
						if (c.randomMap == 0) {
							if (Server.HungerGames.gameTimer < 20) {
								c.sendMessage("You can't use this till 10 game seconds have passed!");
								return false;
							}
							if (c.startDeathBattle && !c.inDeathBattle()) {
								c.sendMessage("You can't use this from out of the game area!");
								return false;
							} else if (!c.startDeathBattle && !c.inVarrockPvp()) {
								c.sendMessage("You can't use this from out of the game area!");
								return false;
							}
						} else if (c.randomMap == 1) {
							if (Server.HungerGamesFal.gameTimer < 20) {
								c.sendMessage("You can't use this till 10 game seconds have passed!");
								return false;
							}
							if (c.startDeathBattleFal && !c.inDeathBattle()) {
								c.sendMessage("You can't use this from out of the game area!");
								return false;
							} else if (!c.startDeathBattleFal && !c.inFalPvp()) {
								c.sendMessage("You can't use this from out of the game area!");
								return false;
							}
						} else if (c.randomMap == 2) {
							if (Server.HungerGamesCan.gameTimer < 20) {
								c.sendMessage("You can't use this till 10 game seconds have passed!");
								return false;
							}
							if (c.startDeathBattleCan && !c.inDeathBattle()) {
								c.sendMessage("You can't use this from out of the game area!");
								return false;
							} else if (!c.startDeathBattleCan && !c.inCanPvp()) {
								c.sendMessage("You can't use this from out of the game area!");
								return false;
							}
						} else if (c.randomMap == 3) {
							if (Server.HungerGamesMisc.gameTimer < 20) {
								c.sendMessage("You can't use this till 10 game seconds have passed!");
								return false;
							}
							if (c.startDeathBattleMisc && !c.inDeathBattle()) {
								c.sendMessage("You can't use this from out of the game area!");
								return false;
							} else if (!c.startDeathBattleMisc && !c.inMiscPvp()) {
								c.sendMessage("You can't use this from out of the game area!");
								return false;
							}
						}
							...
							for (int player : map) {
								if (Server.playerHandler.players[player] != null) {
									...
									if (c.taggedPlayer.equals(o.playerName)) {
										...
										if (o.stuck) {
											c.sendMessage("The player is currently using the ::unstuck command.");
											break;
										}
										o.getPA().movePlayer(c.absX, c.absY, 0);
										c.sendMessage("You use the Tagger device to teleport @blu@" + c.taggedPlayer + "@bla@ to you!");
										c.sendMessage("The device has " + (c.taggedUses - 1) + " uses left.");
										o.sendMessage("You feel a strange force teleport you! Becareful!");
				...
				for (int BullShit : Config.FailItem) {
					if(wearID == BullShit){
						c.sendMessage("Non-working item you can't wear it");
						return false;
					}
				}
...
c.isFullBody = Item.isFullBody(c.playerEquipment[c.playerChest]);
	...
	**/
	...
	public void removeItemChaos(int wearID, int slot) {
		synchronized(c) {
			if(c.getOutStream() != null && c != null) {
				if(c.playerEquipment[slot] > -1){
					c.playerEquipment[slot]=-1;
	...
	*/
	...
	public boolean bankItem(int itemID, int fromSlot, int amount){
		if (c.isBanking == false) {
			...
			c.sendMessage("Don't try to bank glitch :)");
			return false;
		}
		if(c.inTrade) {
	...
	**/
	...
	public boolean playerHasNewItems() 
	{
		boolean hasold;
		//c.sendMessage(" playerItems.length: " +c.playerItems.length);
		for (int i = 0; i < c.playerItems.length; i++) 
		{
			if (c.playerItems[i] != 0) 
			{
			//c.sendMessage("Nuna er eg med item: "+(c.playerItems[i]-1) );
				// F
				hasold = false;
				for (int BullShit : Config.OLD_WEAPONS) 
				{
					if ((c.playerItems[i]-1) == BullShit) 
					{
						//c.sendMessage("Can use the item");
						hasold = true;
					}
				}
				if (hasold == false)
				{
						c.sendMessage(" You can't enter with the following item : "+(c.getItems().getItemName(c.playerItems[i]-1)));
		...
		return false;
			...
			public boolean playerHasNewWeapons( ) 
	{
		boolean hasold;
         for (int lol = 0; lol <= 13; lol++) {
		{
			if (c.playerEquipment[lol] != -1) {

				hasold = false;
				for (int BullShit : Config.OLD_WEAPONS) {
					if (c.playerEquipment[lol] == BullShit) {
						hasold = true;
					}
				}
				if (hasold == false) {
			c.sendMessage(" You can't enter with the following item : "+(c.getItems().getItemName(c.playerEquipment[lol])));
	...
	**/
	...
	public void makeGodsword(int i) {
		...
		if (playerHasItem(11690) && playerHasItem(i)) {
			...
			if (c.HGAttack && c.achievements[7][0] == 0) {
				c.achievements[7][1] = 1;
